  ...
  for (k in 1:N){
    ...
    for (i in 1:length(vthree)){ 
      if (vthree[i] == 0){
        ...
        #NOTE: experiencing the following error message(s) here with no effect on output
    ...
    ##cumlativeinfections = 1:length(infections)
    ##plot(infections,cumlativeinfections, type = "l", xlab = "time (days)", ylab = "cumulative number of infections")
